...
void loop() {
  ...
  if (Serial.available()) {
    ...
    for (fading = 0; fading < 255; fading = fading + 5) {
      for (st = 1; st < 4; st++) {
        //array_LED[2] = 85;
        //ligh_pulse();
        char str[16];
...
void ligh_pulse() {
  ...
  int oldH = array_LED[2];
  newH = array_LED[2] - fading; //random(255-fading, 255-fading*2);
